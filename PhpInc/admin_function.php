...
function getTemplateContent($templateFileName){
    ...
    }else if( checkFile($customTemplateFile) == true ){
...
function showColumnList( $parentid, $thisPId, $nCount, $action){
    ...
    $fieldNameList = getHandleFieldList($GLOBALS['db_PREFIX'] . 'webcolumn', '字段列表') ;
    $splFieldName = aspSplit($fieldNameList, ',') ;
    $sql = 'select * from ' . $GLOBALS['db_PREFIX'] . 'webcolumn where parentid=' . $parentid ;
    //处理追加SQL

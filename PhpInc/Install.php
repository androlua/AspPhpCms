...
<?PHP
 	...
 	$conn=OpenConn(); 
	$mydbcharset = 'gbk';				//编码
 	$mydbpre=@$_REQUEST['db_PREFIX'];						//表前面的前缀
...
"{$mydbpre}lineqq" => "CREATE TABLE `{$mydbpre}lineqq` (
`id` int(5) unsigned NOT NULL auto_increment,
...
"{$mydbpre}linevote" => "CREATE TABLE `{$mydbpre}linevote` (
`id` int(5) unsigned NOT NULL auto_increment,
`title` varchar(255) NOT NULL default '',
...
"{$mydbpre}systemlog" => "CREATE TABLE `{$mydbpre}systemlog` (
`id` int(5) unsigned NOT NULL auto_increment,
...
if($mydbpre<>''){
	...
	if(checkfile($configPath)==true){
		...
		$content=replace($content,$s,'$db_PREFIX = \''.$mydbpre.'\'');
	...
	if(@$_POST['loginname']<>'' && @$_POST['loginpwd']<>''){
			$conn->query('insert into '.$mydbpre.'admin (username,pwd,flags) values(\''.@$_POST['loginname'].'\',\'' . myMD5(@$_POST['loginpwd']) . '\',\'|*|\')') ;
	}else{
		...
		foreach( $splStr as $s){
			...
			$conn->query('insert into '.$mydbpre.'admin (username,pwd,flags) values(\''.$s.'\',\'' . myMD5($s) . '\',\''.$flags.'\')') ;
		}
	}
	
	$conn->query('insert into '.$mydbpre.'website (webtitle) values(\'默认\')') ;

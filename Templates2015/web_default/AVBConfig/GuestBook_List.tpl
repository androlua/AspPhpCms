...
.guestbooklist .gbitem{
    border:1px solid #B4B4B4;
    line-height:22px;
    font-size:14px;
    margin:0 0 10px 0;    
}
.guestbooklist .gbtitle{
    padding:0 0 0 10px;
    color:#000;
    line-height:30px;
    background-color:#EBEBEB;
}
.guestbooklist .gbmessage{
    border-top:1px solid #B4B4B4;
    padding:10px;
}
.guestbooklist .gbreply{
    border-top:1px solid #B4B4B4;
    padding:10px;
}
...
<!--#Module module_Right start#-->
    ...
    <div class="clear10"></div>    
    {$ReadColumeSetTitle title='留言' style='22' contentwidth='720' moreclass='leftmore' morestr='More' moreurl=' ' stylevalue='0' value='[$读出内容 block=\'BlockName留言\' file=\'\'$]'$}
    <div class="clear10"></div>
...
<R#读出内容BlockName详细内容 start#>
            ...
            [list] 
                <li class="gbitem">
                    <div class="gbtitle">姓名：[$guestname$] 于 [$adddatetime time format_time='2'$] 发表</div>
                    <div class="gbmessage">[$bodycontent$]</div>
                    <div class="gbreply"><b>管理员</b><br><font color="#339900">回复：</font>[$reply$]</div>
                </li> 
            [/list]
            </ul>
            <div>[$pageInfo$]</div>
...
<R#读出内容BlockName留言 start#>
    <form class="comment" name="formmessage" method="post" action="[$WEB_VIEWURL$]?act=savedata&stype=guestbook&columnid=[$glb_columnId$]" target="_blank" onSubmit="return formmessageSubmit()">
    ...
    内容：<textarea name="bodycontent" placeholder="我也来说两句..." class="inputstyle"></textarea>
    <input type="submit" class="mybtn" value="提交留言"></form>
<script type="text/javascript">
function formmessageSubmit(){
	if(formmessage.guestname.value==''){
		formmessage.guestname.focus();
		alert('姓名不能为空');
		return false;
	}
	if(formmessage.tel.value==''){
		formmessage.tel.focus();
		alert('电话不能为空');
		return false;
	}
	if(formmessage.bodycontent.value==''){
		formmessage.bodycontent.focus();
		alert('留言内容不能为空');
		return false;
	}
}

...
ContextMenu.display=function(evt,popupoptions){ 
	...
	var D = document 
	var newx=Math.max(D.body.scrollLeft, D.documentElement.scrollLeft)+ mousePos.x
	var newy=Math.max(D.body.scrollTop, D.documentElement.scrollTop)+ mousePos.y
	...
	//c+="x=" + x +",y=" + y + "\n" + document.documentElement.scrollTop ; alert(c)
...
ContextMenu.populatePopup=function(arr,win){
	...
	//if (doc.getElementsByTagName("LINK").length == 0){
		doc.open();
		doc.write('<html><head><link rel="StyleSheet" type="text/css" href="/Jquery/contextmenu.css"></head><body></body></html>');
		doc.close();
	//}
	for(i=0;i<alen;i++)	{
